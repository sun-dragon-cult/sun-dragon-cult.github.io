...
![Supported Foundry Versions](https://img.shields.io/endpoint?url=https://foundryshields.com/version?url=https://github.com/sun-dragon-cult/fvtt-system-rqg/releases/download/v4,0.0/system.json)
![Download Count](https://img.shields.io/github/downloads/sun-dragon-cult/fvtt-system-rqg/v4.0.0/rqg.zip)

This long overdue release completely rewrites the handling of rolls. It introduces multiple new
Foundry rolls:
...
Since a lot of the roll handling has changed, it is recommended to test the new system in a safe
environment before using it in a live game.
...
- **Hit Location Formula** - The dropdown contains 3 options; the normal 1d20, 1d10 & 1d10+10 to
  allow attacking from below or above. If "Aimed Blow" (below) is selected, this will be disabled
...
- **Master Opponent Penalty** - if the attacker has a chance above 100% you can get a penalty to the
  defence here. If you have over 100% on the defence the attacking roll can also get a penalty.

At the bottom you can see the target percentage that you will roll against, as well as a button to
do the selected defence. Pressing that button will update the chatmessage with more info like this:

![](chat-defended.png)

The rolls can be expanded to reveal the modifiers applied. Depending on the outcome you will see a
different things, but in this example clicking on "Roll Damage & Hit Location" will update the
chatmessage with those rolls as well:
